...
void transport01a()
{
  ...
  TGeoVolume *magnetB = geom->MakeBox ("magnetB",med1,240,40,120);
  ...
  vec[4] = gRandom->Gaus(0.0,0.001); // py
...
void magnet_transport(const Double_t ptot, const Double_t field)
{
  ...
  // calculate exactly full angel
  Double_t master[3] = { px, py, pz };
  Double_t local[3];
  gGeoManager->MasterToLocalVect(master,local);
  px = local[0]; py = local[1]; pz = local[2];
  Double_t pt       = TMath::Sqrt(px*px + pz*pz); // invariant
  Double_t radius   = pt/b3;
  TGeoBBox *mag_box = (TGeoBBox *)(gGeoManager->GetCurrentVolume()->GetShape());
  Double_t length   = 2.0*mag_box->GetDZ();
  Double_t tx       = px/pz;
  Double_t a_in     = -tx/TMath::Sqrt(1+tx*tx); // <=> zeta from transport2.C
  //   !!! a_in     = -px/pt !!! but sign depend too on pz !!!
  //       a_in     =>  sin(input angel)
  Double_t a_in2    = -TMath::Sign(px/pt,px/pz);
  a_in = a_in2;
  Double_t a_out    = a_in + length/radius; // <=> zetad
  //       a_out    => sin(output angel)
  a_in  = TMath::ASin(a_in);
  a_out = TMath::ASin(a_out);
  Double_t a_full = a_out-a_in;
  px = master[0]; py = master[1]; pz = master[2]; // restore to master
  ...
  for (Int_t i = 1; i < 9999; i++) {
    ...
    if (gGeoManager->FindNode(np[0], np[1], np[2]) == c_n) { // enlarge angel
      ...
      if (back_transport) {
        ...
        cout << "full angel         = " << full_angel*TMath::RadToDeg() << endl;
        cout << "compute full angel = " << a_full*TMath::RadToDeg() << endl;
        cout << endl;
        return;
      }
    }
