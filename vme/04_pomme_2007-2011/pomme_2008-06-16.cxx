...
Text_t tt[80];
  ...
  fp[7] = fopen("/daq/nis/alpha.dat","r");

  UChar_t  work1;
  UShort_t width, wire, wire0, nb, ip;
  UShort_t ampl, hit, delta, nbr;
  long fpos;
  //  LInt_t work, work0;
  ...
  for ( i0 = 0; i0<Nevents; i0++) { 
   ...
   ii = 0;
	...
	if(((Data[3] &  0xF0) == 0x80) && ((Data[2] & 0x0F) != 0x0A)) i1 += 512;	//1024
	if( Data[3] == 0xA0) iev7++;		
      }
...
//------------------------------------------------------------  Create 1
   ...
   if(iev7 != ievents) {printf("Error in tdcs %d %d\n",iev7,ievents); ret = 7; goto error;}  
...
int main(int argc, char **argv)
{
  ...
  new TGepom("jun08", "gem", "alp01", "qwertyasdf");
