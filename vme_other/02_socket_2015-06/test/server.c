...
#define BUF_SIZE 500 // same as in client
...
int main(int argc, char *argv[])
{
  ...
  while (1) {
    ...
    printf("receiving ...\n");
    bytes_recieved = recv(newsfd, buf, BUF_SIZE-1, 0); // maybe set socket in non-blocking mode ?!
    if (bytes_recieved == -1) perror("recv failed");
    else if (bytes_recieved == 0) fprintf(stderr, "client closed connection (shutdown)\n");
    else printf("from client received %d bytes: %s\n", bytes_recieved, buf);
